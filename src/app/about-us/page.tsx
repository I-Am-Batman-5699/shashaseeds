...
const ScrollFadeIn = ({ children, direction = 'up', delay = 0, className = '', id, scrollMarginClass = "scroll-mt-24" }: { children: React.ReactNode, direction?: 'up' | 'left' | 'right' | 'down', delay?: number, className?: string, id?: string, scrollMarginClass?: string }) => {
    ...
    return (
        <div
            id={id}
            ref={ref}
            className={`${baseTransition} ${isVisible ? visibleClasses : transformClasses} ${scrollMarginClass} rounded-2xl shadow-xl md:p-4 p-2 inset-shadow-sm inset-shadow-indigo-900/50 dark:inset-shadow-indigo-900/50`}
